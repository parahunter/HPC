...
def main():
	...
	littlem=4;
	ns = [(int(math.ceil(math.sqrt(littlem*littlem+x*1024/8))-littlem)) for x in memory_approx]
	print ns
	...
	for i in range(len(memory_approx)):
		...
		for test in implementation:
			...
			process = Popen(("./matmult_f.studio %s %d %d %d" % (test,littlem,ns[i],ns[i]+1)).split() ,stdout= PIPE,bufsize=-1) 
