...
memory_approx =   [(x) for x in [16, 32, 64, 128, 256 , 512, 1024, 2048]]#, 4096, 8192, 16384]] 
implementation = ["lib","nmk","nkm","knm","kmn","mnk","mkn"] # "blk"] (when ready)
times = 1
...
def main():
	...
	for i in range(len(memory_approx)):
		...
		for test in implementation:
			...
			process = Popen(("./matmult_f.studio %s %d %d %d" % (test,ns[i],ns[i],ns[i])).split() ,stdout= PIPE,bufsize=-1) 
